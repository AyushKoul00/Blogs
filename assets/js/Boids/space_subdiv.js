...
class SubDiv {
	...
	update(item) {
		const i =
			Math.floor(item.position.y / this.size) * this.rowLength +
			Math.floor(item.position.x / this.size);
		// if(item.debug)
		// 	console.log(item instanceof Agent);
		// if (item.debug) console.log(item.bucket);

		//if item (agent) is in a different bucket, then update it:
		if (i !== item.bucket) {
			//if item is already in a bucket, remove it from the old bucket
